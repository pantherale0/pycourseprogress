...
class Member:
    ...
    def get_class(self, class_id: int = -1, class_name: str = "") -> Class:
        ...
        data = [x for x in self.classes
                if x.class_id == class_id or x.class_name == class_name]
        if len(data) == 1:
            return data[0]
    ...
    async def update(self):
        ...
        # now do the classes
        response = await self._api.send_http_request(
            endpoint="get_classes_history",
            MEMBER_ID=str(self.member_id)
        )
        response = response["response"]["content"]
        for member_class in response:
            if member_class["class_id"] not in self.discovered_classes:
                _LOGGER.debug("Discovered new class %s for member %s",
                              member_class["class_id"], self.member_id)
                self.classes.append(await Class.create(
                    api=self._api,
                    class_id=member_class["class_id"],
                    member_id=self.member_id,
                    class_history_data=member_class
                ))
                self.discovered_classes.append(
                    member_class["class_id"]
                )
            else:
                await self.get_class(member_class["class_id"]).update(
                    class_history_data=member_class
                )
